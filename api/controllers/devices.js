...
exports.devices_get_all = (req, res, next) => {
  if (!req.AuthData.admin) {
    ...
    Device.find().select('_id url name description gateway mac found batt rssi screen initcode created_at updated_at').populate('gateway', '_id url name mac').exec().then((docs) => {
...
exports.devices_get_one = (req, res, next) => {
  ...
  Device.findById(_id).select('_id url name description mac mac found batt rssi initcode screen display userGroup active_image created_by created_at updated_at').populate('display', '_id url name').populate('gateway', '_id url name').populate('created_by', '_id url name').populate('updated_by', '_id url name').populate('resolution', '_id url name').populate('location', '_id url name').populate('userGroup', '_id url name').populate('active_image', '_id url name').exec().then(doc => {
...
exports.device_update = (req, res, next) => {
  if (!req.checkAuth.admin) {
    ...
    // send response
      .then((res) => Device.findById(_id).exec()).then(doc => {
      const result = {
        _id: doc._id,
        url: doc.url,
        name: doc.name,
      ...
      res.status(200).json({message: 'Success at updating a device from the collection', success: true, result: result})
    })
    // catch any errors
      .catch(err => {
      console.log(err)
...
exports.device_delete = (req, res, next) => {
  if (!req.AuthData.admin) {
    ...
    const _id = req.params.id
    Device.remove({_id: _id}).then((res) => result = res)
    // update userGroups involved
      .then(() => {
      return UserGroup.updateMany({
        devices: _id
