...
exports.images_get_all = (req, res, next) => {
  if (!req.AuthData.admin) {
    ...
    Image.find().select('_id name description tags url created_at updated_at').exec().then(docs => {
      setTimeout(() => {
...
exports.images_get_one = (req, res, next) => {
  ...
  Image.findById(query).select('_id url name description created_by updated_by extension path size src_url color_profile resolution category groups displays tags userGroup created_at updated_at').populate('displays', '_id url name').populate('groups', '_id url name').populate('resolution', '_id url name size').populate('created_by', '_id url name').populate('updated_by', '_id url name').populate('userGroup', '_id url name description').exec().then(doc => {
    if (doc) {
...
exports.image_create = (req, res, next) => {
  ...
  // send response
    .then(() => Image.findById(_id).select('_id url name description tags created:at').exec()).then((doc) => {
    res.status(201).json({message: 'Success at adding a new image to the collection', success: true, result: doc})
  })
  // catch any errors
    .catch(err => {
...
exports.image_update = (req, res, next) => { // get id from request parameters
  ...
  // remove display
  Display.find(query).remove().exec().then((res) => result = res)
  ...
  // send response
    .then(() => Device.find({userGroup: req.AuthData.userGroup}).select('_id url name description display updated_at created_at').populate('display', '_id url name description').exec()).then((doc) => {
    res.status(200).json({message: 'Success at removing a display from the collection', success: true, resourceId: _id, devices: doc})
  }).catch(err => {
    console.log(err)
    res.status(500).json({error: err})
  })
...
exports.image_delete = (req, res, next) => { // get id from request parameters
  ...
  // remove image
  Image.find(query).remove().exec().then(() => {
    return Display.updateMany({
      images: _id
    }, {
      $pull: {
  ...
  }).then(() => res.status(200).json({message: 'Success at removing an image from the collection', success: true, resourceId: _id})).catch(err => {
    res.status(500).json({error: err})
  })
}
...
exports.image_upload = (req, res, next) => {
  const _id = req.params.id
  Image.findById(_id).then((doc) => doc.path && fs.unlink(doc.path)).then(() => { //if path exists, remove previous file
    ...
    // send response
      ...
      res.status(200).json({message: 'Success at removing a display from the collection', success: true, resourceId: _id, resource: doc})
    })
  }).catch(err => {
    console.log(err)
    res.status(500).json({message: 'Internal Server Error', error: err})
